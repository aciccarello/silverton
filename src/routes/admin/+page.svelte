<script lang="ts">
  ...
  async function handleRollback(id: number) {
    const confirmed = await confirmStore.confirm(
      "Are you sure you want to rollback to this state? All progress after this point will be lost.",
    );
    if (!confirmed) {
      return;
    }
  ...
  async function handleResetGame() {
    const confirmed = await confirmStore.confirm(
      "Are you sure you want to start a new game? This will clear the current player list and active game state, but the current game history will be preserved for audit/rollback.",
    );
    if (!confirmed) return;
...
<section class="card" style="margin-bottom: var(--spacing-lg);">
  <div
    style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: var(--spacing-md); border-bottom: 1px solid var(--color-border); padding-bottom: var(--spacing-sm);"
  >
            ...
            <tr class:current-state={i === 0}>
              ...
              <td>
                <span class="user-badge"
                  >{(record.state as any)?.lastModifiedBy || "Unknown"}</span
                >
              </td>
              <td class="action-cell"
                >{(record.state as any)?.lastModifiedAction || "-"}</td
              >
              <td style="text-transform: capitalize;"
                >{(record.state as any)?.currentPhase || "setup"}</td
              >
    ...
    {#if data.pagination.totalPages > 1}
      <div class="pagination-controls">
        <a
          ...
          onclick={(e) =>
            data.pagination.currentPage <= 1 && e.preventDefault()}
        >
          &laquo; Newer
        </a>

        <div class="page-indicator">
          Page <strong>{data.pagination.currentPage}</strong> of {data
        ...
        <a
          href="?page={data.pagination.currentPage + 1}"
          class="btn btn-sm btn-outline"
          class:disabled={data.pagination.currentPage >=
