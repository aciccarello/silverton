...
const turn1GlobalStmt = db.prepare(
  "SELECT 1 FROM global_market_state WHERE turn_number = 1 LIMIT 1",
);
if (!turn1GlobalStmt.get()) {
  const insGlobal = db.prepare(
    "INSERT INTO global_market_state (turn_number, resource, price_index) VALUES (1, ?, ?)",
  );
  ...
  const cityIds = new Map(
    (
      db.prepare("SELECT id, name FROM cities").all() as {
        id: number;
        name: string;
      }[]
    ).map((r) => [r.name, r.id]),
...
if (count === 0) {
  const insertStmt = db.prepare(
    "INSERT INTO state_history (state_json) VALUES (?)",
  );
  insertStmt.run("{}");
}
...
export function loadState(): string {
  const stmt = db.prepare(
    "SELECT state_json FROM state_history ORDER BY id DESC LIMIT 1",
  );
...
export function getHistory(
  limit: number = 50,
  offset: number = 0,
): Array<{ id: number; timestamp: string; state_json: string }> {
  const stmt = db.prepare(
    "SELECT id, timestamp, state_json FROM state_history ORDER BY id DESC LIMIT ? OFFSET ?",
  );
...
export function rollbackTo(
  ...
  try {
    const stmt = db.prepare(
      "SELECT state_json FROM state_history WHERE id = ?",
    );
...
export function getGlobalMarketState(turnNumber: number): GlobalMarketRow[] {
  const stmt = db.prepare(
    "SELECT turn_number, resource, price_index FROM global_market_state WHERE turn_number = ?",
  );
